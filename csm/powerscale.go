...
func (p PowerScaleStruct) GetLogs(namespace string, optionalFlag string, noOfDays int, driverStorageSystem int) {
	...
	if err != nil {
		pscLog.Fatalf("Getting all pods failed with error: %s", err.Error())
	}
